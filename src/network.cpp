...
void Network::initialiseDescriptors(int maxCnxs) {
    ...
    //cout << "Fuck ? " << nodes[7640].netCnxs.n << endl;
    for(int i=0; i<nodes.n; ++i){
        for(int j=0; j<nodes[i].netCnxs.n; ++j){
//            cout << i << " " << nodes[i].netCnxs[j] << " " << nodes[i].netCnxs.n << " " << nodes[nodes[i].netCnxs[j]].netCnxs.n << endl;
...
Network::Network(string prefix, int maxNetCnxsA, int maxDualCnxsA) {
 ...
 //   cout << "Aux File is        : " << prefix+"_aux.dat" << endl;
    ...
    if (maxNetCnxs<maxNetCnxsA) 	maxNetCnxs	=maxNetCnxsA;
    if (maxDualCnxs<maxDualCnxsA)	maxDualCnxs	=maxDualCnxsA;
    ...
    ss>>rpb[0];
    ss>>rpb[1];
//    cout << "Boundary Conditions : " << pb[0] << " " << pb[1] << endl;
//    cout << "R Boundary Conditions : " << rpb[0] << " " << rpb[1] << endl;
...
//        cout << "First Cnx A: " << cnx << endl;

//        ss.str(line);
        //cout << "Connection line : " << line << " " << ss << endl;
        //ss>>cnx;
        //nodes[i].netCnxs.addValue(cnx);
//        cout << "First Cnx B: " << cnx << endl;

        while(ss>>cnx) {
//            cout << i << "-->" << cnx << endl;
            nodes[i].netCnxs.addValue(cnx);
        }
//        cout << "Connection Number : " << i << " " << nodes[i].netCnxs.n << endl;
    }
    netFile.close();
...
//    ifstream ignoreFile(prefix+"_ignore.dat", ios::in);
...
Network::Network(VecR<Node> nodesA, VecF<double> pbA, VecF<double> rpbA, string type, int maxNetCnxsA, int maxDualCnxsA){
    ...
    if (verbose){cout << "Generating Triangle Raft" << endl;}
    ...
    if (type == "t" or type == "triangle_raft"){
        if(verbose){cout << "Triangle Raft" << endl;}
        nNodes = nodesA.n*5/2;
        geometryCode = "2DEtr";
        ...
        // Add oxygens
        nodes=VecR<Node>(0,nNodes);
//        cout << "Replicating Old Nodes" << endl;
...
//            cout << "Adding Coords" << endl;
            //nodes[i].crd[0] = nodesA[i].crd[0];
            //nodes[i].crd[1] = nodesA[i].crd[1];
            nodes[i].crd = nodesA[i].crd*SiScaling;
//            cout << "Added Coords" << endl;
        }
//        cout << "Add Oxygen Nodes" << endl;
        ...
        for (int i=0; i<nodesA.n; ++i){
            ...
            for (int j=0; j<nodes[i].netCnxs.n;++j){
                ...
                if (atom1>i && atom1<nodesA.n){
                    ...
                    if(verbose){cout << "    " << atom1 << endl;}
                    v[0] = crds1[0] - crds2[0];
                    if          (v[0] > pb[0]/2.0){ v[0] -= pb[0];}
                    else if     (v[0] < -pb[0]/2.0){v[0] += pb[0];}
                    v[1] = crds1[1] - crds2[1];
                    if          (v[1] > pb[1]/2.0){ v[1] -= pb[1];}
                    else if     (v[1] < -pb[1]/2.0){v[1] += pb[1];}

/*
                    v[2] = crds1[2] - crds2[2];
                    if          (v[2] > pb[2]/2.0){ v[2] -= pb[0];}
                    else if     (v[2] < -pb[2]/2.0){v[2] += pb[0];}
*/
                    ...
                    if          (crds[0] >  pb[0]){crds[0] -= pb[0];}
                    else if     (crds[0] < -pb[0]){crds[0] += pb[0];}
                    if          (crds[1] >  pb[1]){crds[1] -= pb[1];}
                    else if     (crds[1] < -pb[1]){crds[1] += pb[1];}


                    Node node(OxygenAtom,maxNetCnxs*2,maxDualCnxs,0);
                    nodes.addValue(node);
                    if (verbose) {cout << "Adding Oxygen Crds" << endl;}

                    nodes[OxygenAtom].crd = crds;
//                    nodes[OxygenAtom].crd[0] = crds[0];
//                    nodes[OxygenAtom].crd[1] = crds[1];
//                    nodes[OxygenAtom].crd[2] = crds[2];

//                    nodes[OxygenAtom].crd[0] = crds[0];
                    if (verbose) {cout << "Swapping out Cnxs" << endl;}

//                    nodes[i].netCnxs.delValue(nodes[i].netCnxs[j]);
//                    nodes[i].netCnxs.addValue(OxygenAtom);


                    nodes[atom0].netCnxs.swapValue(atom1, OxygenAtom, true);
                    nodes[atom1].netCnxs.swapValue(atom0, OxygenAtom, true);

                    if (verbose) {cout << "Adding Oxygen Cnxs" << endl;}
                    nodes[OxygenAtom].netCnxs.addValue(atom0);
                    nodes[OxygenAtom].netCnxs.addValue(atom1);

                    if (verbose){cout << "Oxygen Atom " << OxygenAtom << endl;}
        ...
        if (verbose) {cout << "Add Oxygen Triangles" << endl;}
...
//    newoutputPrefixfolder = "./output_files";
...
//    newoutputPrefixfolder = "./output_files";
    ...
    for (int i=0;i<nodes.n;++i){
        ...
        if (O[0]<0) O[0] += dim_x;
        else if (O[0]>dim_x) O[0] -= dim_x;

        O[1] = nodes[i].crd[1]*si_si_distance;

        if (O[1]<0) O[1] += dim_y;
        else if (O[1]>dim_y) O[1] -= dim_y;
    ...
    for(int i=0; i<nodes.n; ++i) {
        for (int j = 0; j < nodes[i].netCnxs.n; ++j) {
            if (nodes[i].netCnxs[j] > i) {
                ...
                if (diff[0] > micx) diff[0] -= dim_x;
                else if (diff[0] < -micx) diff[0] += dim_x;
                if (diff[1] > micy) diff[1] -= dim_y;
                else if (diff[1] < -micy) diff[1] += dim_y;
...
void Network::writeBilayerB(string prefix){
...
//
//    cout << "##### Rings" << endl;
//    ofstream siringsFile(newoutputPrefixfolder+"/rings_si.dat",     ios::in | ios::trunc);
//    ofstream oringsFile(newoutputPrefixfolder+"/rings_o.dat",       ios::in | ios::trunc);
//    ofstream sioringsFile(newoutputPrefixfolder+"/rings_si_o.dat",  ios::in | ios::trunc);
//
//    VecR<int> Si_Ring (100);
//    VecR<int> O_Ring (100);
//    VecR<int> Si_O_Ring (200);
//
//    int si_0, si_1;
//
//    for (int i=0;i<nodes.n;i++){
//        cout << "Si UPPER rings" << endl;
//        for (int j=0;j<nodes[i].dualCnxs.n;j++){
//            cout << "  " << j << endl;
//            Si_Ring[j] = 2*nodes[i].dualCnxs[j];
//            cout << "    " << Si_Ring[j] << endl;
//            if (j==0) si_0 = nodes[i].dualCnxs[nodes[i].dualCnxs.n-1];
//            else si_0 = nodes[i].dualCnxs[j-1];
//            cout << "      " << si_0 << endl;
//            si_1 = nodes[i].dualCnxs[j];
//            cout << "      " << si_1 << endl;
//            cout << 2*si_1 << "---->" << Si_O_Ring[2*j] << endl;
//            Si_O_Ring[2*j] = 2*si_1;
//
//            for (int k0=0;k0<4;++k0){
//                for (int k1=0;k1<4;++k1){
//                    cout << k0 << "    " << k1 << endl;
//                    cout << "    " << nodes[si_0].oxyCnxs[k0] << " vs " << nodes[si_1].oxyCnxs[k1] << endl;
//                    if (nodes[si_0].oxyCnxs[k0]==nodes[si_1].oxyCnxs[k1]){
//                        O_Ring[j] = nodes[si_0].oxyCnxs[k0];
//                        Si_O_Ring[2*j+1] = nodes[si_0].oxyCnxs[k0];
//                    }
//                }
//            }
//        }
...
//        for (int j=0;j<nodes[i].dualCnxs.n;j++){
//            siringsFile << setw(20) << left << Si_Ring[j];
//            oringsFile << setw(20) << left << O_Ring[j];
//            sioringsFile << setw(20) << left << Si_O_Ring[2*j];
//            sioringsFile << setw(20) << left << Si_O_Ring[2*j+1];
//
//            siringsFile << endl;
//            oringsFile << endl;
//            sioringsFile << endl;
...
//        //////////////////////////////////////////////////////////////////////////
//        //////////////////////////////////////////////////////////////////////////
//        cout << "Si LOWER rings" << endl;
//        for (int j=0;j<nodes[i].dualCnxs.n;j++){
//
//            Si_Ring[j] = 2*nodes[i].dualCnxs[j]+1;
//
//            if (j==0) si_0 = nodes[i].dualCnxs[nodes[i].dualCnxs.n-1];
//            else si_0 = nodes[i].dualCnxs[j-1];
//            si_1 = nodes[i].dualCnxs[j];
...
//
//    //Write rings
//    // more challenging as rings not defined, just dual connections
//    // Each node is connected to 3 ring centers (dual coordinates)
//    // loop through and compare to list as long as number of nodes
//
...
//
//
//    ofstream ljpairsFile(newoutputPrefixfolder+"/lj_pairs.dat",     ios::in | ios::trunc);
//    VecF<int> Pair(2);
//
//    int lj_count = 0;
//    for (int i=0;i<nodes.n;++i){
...
//    for (int i=0;i<nodes.n;++i){
//        for (int j=0;j<nodes[i].dualCnxs.n;j++){
//            for (int k=j;k<nodes[i].dualCnxs.n;k++){
...
//    }
//
//    ljpairsFile << lj_count << endl;
//
//    for (int i=0;i<nodes.n;++i){
//        Pair[0] = 2*i+1;
//        Pair[1] = 2*i+2;
//        ljpairsFile << setw(20) << left << Pair[0];
//        ljpairsFile << setw(20) << left << Pair[1] << endl;
//
//        Pair[0] = 2*i+2;
//        Pair[1] = 2*i+1;
//        ljpairsFile << setw(20) << left << Pair[0];
...
//            for (int k=j;k<nodes[i].dualCnxs.n;k++){
////                if (rings[i].nodes[j]>rings[i].nodes[k]) {
//                    Pair[0] = 2 * nodes[i].dualCnxs[j]+1;
//                    Pair[1] = 2 * nodes[i].dualCnxs[k]+1;
//                    ljpairsFile << setw(20) << left << Pair[0];
//                    ljpairsFile << setw(20) << left << Pair[1] << endl;
//                    Pair[0] = 2 * nodes[i].dualCnxs[j] + 2;
//                    Pair[1] = 2 * nodes[i].dualCnxs[k] + 2;
//                    ljpairsFile << setw(20) << left << Pair[0];
//                    ljpairsFile << setw(20) << left << Pair[1] << endl;
////                }
//            }
