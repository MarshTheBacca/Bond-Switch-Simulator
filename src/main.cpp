...
int main(int argc, char *argv[]) {
  ...
  lammpsBondAngle.write("bond_coeff 1 " +
                        std::to_string(inputData.harmonicBondForceConstant) +
                        " 1.000");
  lammpsBondAngle.write("angle_style cosine/squared");
  lammpsBondAngle.write("angle_coeff 1 " +
                        std::to_string(inputData.harmonicAngleForceConstant) +
                        " 120");
  ...
  logger->debug("Monte carlo initial log temperature: {}",
                inputData.startTemperature);
  logger->debug("Monte carlo final log temperature: {}",
                inputData.endTemperature);
  logger->debug("Monte carlo log temperature increment: {}",
                inputData.temperatureIncrement);
  logger->debug("Monte carlo steps per increment: {}",
                inputData.stepsPerTemperature);
  ...
  if (inputData.isFromScratchEnabled) {
    ...
    network = LinkedNetwork(
        inputData.inputFolder, inputData.inputFilePrefix,
        inputData.minCoordination, inputData.maxCoordination,
        inputData.minRingSize, inputData.maxRingSize,
        inputData.isSimpleGrapheneEnabled, inputData.isTriangleRaftEnabled,
        inputData.isBilayerEnabled, inputData.isTersoffGrapheneEnabled,
        inputData.isBNEnabled, inputData.isRestartUsingLAMMPSObjectsEnabled,
        logger);
  }
  ...
  network.initialisePotentialModel(
      inputData.harmonicAngleForceConstant, inputData.harmonicBondForceConstant,
  ...
  std::string prefixOut =
      inputData.outputFolder + "/" + inputData.outputFilePrefix;
  ...
  try {
    ...
    if (inputData.isSpiralEnabled) {
      ...
      for (int i = 0; i < network.networkA.nodes.n; ++i) {
        if (network.rFixed[i] > 1000 / inputData.spiralRadius) {
          ...
          for (int j = 0;
               j < network.networkB.nodes[network.fixedRings[0]].dualCnxs.n;
               ++j) {
            if (i == network.networkB.nodes[network.fixedRings[0]].dualCnxs[j])
              disallowed_node = true;
          }
          if (!disallowed_node) {
            logger->info("Node {} withing radius at distance {}", i,
                         network.rFixed[i]);
            moveStatus = network.SpiralmonteCarloSwitchMoveLAMMPS(
                i, SimpleGrapheneEnergy, TersoffGrapheneEnergy,
            ...
            VecF<double> geomStats = network.getOptimisationGeometry(
                network.networkA, emptyL, emptyA);
    ...
    for (int i = 1; i <= inputData.initialThermalisationSteps; ++i) {
      ...
      if (lammps) {
        moveStatus = network.monteCarloSwitchMoveLAMMPS(
            SimpleGrapheneEnergy, TersoffGrapheneEnergy, TriangleRaftEnergy,
    ...
    int nT = (inputData.endTemperature - inputData.startTemperature) /
             inputData.temperatureIncrement;

    for (int t = 0; t <= nT; ++t) {

      mcT = pow(10, inputData.startTemperature +
                        t * inputData.temperatureIncrement);
      network.mc.setTemperature(mcT);
      ...
      for (int i = 1; i <= inputData.stepsPerTemperature; ++i) {
        ...
        if (i % inputData.structureWriteFrequency == 0 &&
            inputData.isWriteSamplingStructuresEnabled == 1) {
          network.syncCoordinates();
          network.writeXYZ(prefixOut + "_t" + std::to_string(t) + "_" +
                           std::to_string(i));
        }
      }
    }
