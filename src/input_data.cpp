 ...
 */
InputData::InputData(const std::string &filePath, const LoggerPtr &loggerArg)
    : inputFile(filePath), logger(loggerArg) {
...
/**
 * @brief Gets the first word from a line in the input file and increment line
 * number
...
void InputData::readNetworkRestrictions() {
  readSection("Network Restrictions", minRingSize, maxRingSize,
              maximumBondLength, maximumAngle, isFixRingsEnabled);
}

void InputData::readBondSelectionProcess() {
  readSection("Bond Selection Process", randomSeed, randomOrWeighted,
              weightedDecay);
}

void InputData::readTemperatureSchedule() {
  readSection("Temperature", thermalisationTemperature,
              annealingStartTemperature, annealingEndTemperature,
              thermalisationSteps, annealingSteps);
}
...
void InputData::validate() const {
  ...
  checkInRange(maxRingSize, minRingSize, INT_MAX,
               "Maximum ring size must be at least the minimum ring size");

  if (isFixRingsEnabled) {
    checkFileExists(std::filesystem::path("./input_files") / "bss_network" /
                    "fixed_rings.txt");
  }
  // Monte Carlo Process
  ...
  checkInRange(annealingSteps, 0, INT_MAX,
               "Annealing steps must be at least 0");
  checkInRange(thermalisationSteps, 0, INT_MAX,
               "Thermalisation steps must be at least 0");
  checkInRange(maximumBondLength, 0.0, std::numeric_limits<double>::max(),
               "Maximum bond length must be at least 0");
  checkInRange(maximumAngle, 0.0, 360.0,
               "Maximum angle must be between 0 and 360");
  ...
  if (writeMovie && thermalisationSteps + annealingSteps > 2000) {
    throw std::runtime_error("Cannot write a movie file for more than 2000 "
                             "steps because the file would be enormous");
  }
}
