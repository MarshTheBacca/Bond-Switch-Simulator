...
//    VecF<double> crdsA; //copy of coordinates in network A (for efficient geometry optimisation)
    ...
    LinkedNetwork(string prefixFolderIn, string prefixFileIn, string prefixFolderOut, int minA, int maxA, int minB, int maxB, bool isSimpleGraphene, bool isTriangleRaft, bool isBilayer, bool isTersoffGraphene, bool isBN, bool restartLammps);  //construct by loading from files

    void pushPrefix(string prefixin, string prefixout);
    void findIgnoredRings(bool fixed_rings,string filename);

    void makerFixed();
    //Member Functions
    void initialisePotentialModel(Network network, double ak, double bk, double ck=0.0, int convex=0); //set up potential model
    ...
    void initialiseMonteCarlo(Network network, double temperature, int seed=0, bool globalOpt=false); //set up monte carlo
...
//
    ...
    void writeBilayer(string prefix, float cutoff);
};
