...
void VecR<T>::swapValue(T vDel, T vAdd, T vBetween0, T vBetween1) {
    ...
    if (globalVerbose) cout << "dunno" << endl;
    ...
    if(!swap)
        cout << "Sequence not present in vector" << endl;
        throw string("Cannot swap value between values as sequence not present in vector");
}
...
void VecR<T>::insertValue(T vInsert, T vBetween0, T vBetween1) {
    bool globalVerbose=false;
    if (globalVerbose) cout << nMax << endl;
    if(this->n == this->nMax) throw string("Cannot insert to vector to make larger than reserved size");
    bool insert=false;
    int insertPos=-1;
    if (globalVerbose) cout << "Between 0 : " << vBetween0;
    if (globalVerbose) cout << " Between 1 : " << vBetween1;
    if (globalVerbose) cout << "  " << vBetween0 << " " << vBetween1 << endl;

    for(int i=0; i<this->n; ++i){
        if (globalVerbose) cout << i << " " << v[i] << " " << v[(i+1)%this->n]<< endl;
    ...
    ++this->n;
    if (globalVerbose) cout << "inserted" << endl;
}
