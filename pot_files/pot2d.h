...
class HLJ2DP : public BasePotentialModel2D {
  ...
  public:
    ...
    double pby;
    double pbrx;
    double pbry;                // cell lengths and reciprocals
...
// Harmonic Windowed Bonds and Angles, Harmonic Angles, Infinite Cost Line Intersections, Periodic Boundary Conditions
class HWHR2DP : public BasePotentialModel2D {

  public:
    // Constructor
    HWHR2DP(double periodicX, double periodicY);
...
inline bool properIntersectionLines(double &x1a, double &y1a, double &x1b, double &y1b, double &x2a, double &y2a, double &x2b, double &y2b) {
    if (collinearPoints(x1a, y1a, x1b, y1b, x2a, y2a) || collinearPoints(x1a, y1a, x1b, y1b, x2b, y2b) || collinearPoints(x2a, y2a, x2b, y2b, x1a, y1a) || collinearPoints(x2a, y2a, x2b, y2b, x1b, y1b))
        return false;
    return (leftTriangle(x1a, y1a, x1b, y1b, x2a, y2a) ^ leftTriangle(x1a, y1a, x1b, y1b, x2b, y2b)) && (leftTriangle(x2a, y2a, x2b, y2b, x1a, y1a) ^ leftTriangle(x2a, y2a, x2b, y2b, x1b, y1b));
}
