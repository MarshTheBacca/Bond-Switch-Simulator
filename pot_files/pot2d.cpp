...
double HWI2D::bndPotential(double &x0, double &y0, double &x1, double &y1, int &p)
{
    ...
    double r = sqrt((dx * dx + dy * dy));
    if (r < bndP[3 * p + 1])
        return 0.5 * bndP[3 * p] * pow((r - bndP[3 * p + 1]), 2);
    else if (r > bndP[3 * p + 2])
        return 0.5 * bndP[3 * p] * pow((r - bndP[3 * p + 2]), 2);
    else
        return 0.0;
}
...
double HWI2D::angPotential(double &x0, double &y0, double &x1, double &y1, double &x2, double &y2, int &p)
{
    ...
    if (theta < angP[3 * p + 1])
        return 0.5 * angP[3 * p] * pow((theta - angP[3 * p + 1]), 2);
    else if (theta > angP[3 * p + 2])
        return 0.5 * angP[3 * p] * pow((theta - angP[3 * p + 2]), 2);
    else
        return 0.0;
}
                     ...
                     double &fx0, double &fy0, double &fx1, double &fy1, int &p)
{
    ...
    double m = 0.0;
    if (r < bndP[3 * p + 1])
        m = -bndP[3 * p] * (r - bndP[3 * p + 1]) / r;
    else if (r > bndP[3 * p + 2])
        m = -bndP[3 * p] * (r - bndP[3 * p + 2]) / r;
    dx *= m;
    dy *= m;
    fx0 -= dx;
    fy0 -= dy;
...
// ##### HARMONIC WINDOWED BONDS AND ANGLES, INFINITE COST LINE INTERSECTIONS #####
HWHR2DP::HWHR2DP(double periodicX, double periodicY) : BasePotentialModel2D()
{
    pbx = periodicX;
    pby = periodicY;
    pbrx = 1.0 / pbx;
...
// Potential
double HWHR2DP::bndPotential(double &x0, double &y0, double &x1, double &y1, int &p)
{
    double dx = x1 - x0;
    double dy = y1 - y0;
    dx -= pbx * nearbyint(dx * pbrx);
...
double HWHR2DP::angPotential(double &x0, double &y0, double &x1, double &y1, double &x2, double &y2, int &p)
{
    double dx1 = x1 - x0;
    double dy1 = y1 - y0;
    double dx2 = x2 - x0;
...
double HWHR2DP::repPotential(double &x0, double &y0, double &x1, double &y1, int &p)
{
    return 0.0;
}

double HWHR2DP::intPotential(double &x0, double &y0, double &x1, double &y1, double &x2, double &y2, double &x3,
                             double &y3, int &p)
{
    double px1, py1, px2, py2, px3, py3;
    px1 = x1 - x0;
...
double HWHR2DP::gcnPotential(double &x0, double &y0)
{
    return 0.0;
}

// Forces
void HWHR2DP::bndForce(double &x0, double &y0, double &x1, double &y1,
                       double &fx0, double &fy0, double &fx1, double &fy1, int &p)
{
    double dx = x1 - x0;
    double dy = y1 - y0;
...
void HWHR2DP::angForce(double &x0, double &y0, double &x1, double &y1, double &x2, double &y2,
                       double &fx0, double &fy0, double &fx1, double &fy1, double &fx2, double &fy2, int &p)
{
    double dx1 = x1 - x0;
    double dy1 = y1 - y0;
...
void HWHR2DP::repForce(double &x0, double &y0, double &x1, double &y1,
                       double &fx0, double &fy0, double &fx1, double &fy1, int &p)
{
    return;
}

void HWHR2DP::intForce(double &x0, double &y0, double &x1, double &y1, double &x2, double &y2, double &x3, double &y3,
                       double &fx0, double &fy0, double &fx1, double &fy1, double &fx2, double &fy2, double &fx3,
                       double &fy3, int &p)
{
    return;
...
void HWHR2DP::gcnForce(double &x0, double &y0, double &fx0, double &fy0)
{
    return;
}

void HWHR2DP::wrap(VecF<double> &x)
{
    for (int i = 0, j = 1; i < x.n; i += 2, j += 2)
    {
        x[i] -= pbx * nearbyint(x[i] * pbrx) - pbx * 0.5;
