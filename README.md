...
Then enter `source ~/.bashrc` to execute this (and it will automatically execute everytime you log on)
...
In the terminal, enter _build_ and execute the following commands:
```
cmake ../cmake -D BUILD_SHARED=ON
cmake --build . -j X
cmake --install .
```
Where `X` is your number of cores (on Linux you can find this by executing `nproc`), this speeds up the building process.

By default, things will be installed to ~./local/lib, and you need to add another line to _~/.bashrc_ `export PKG_CONFIG_PATH="$HOME/.local/lib/pkgconfig:$PKG_CONFIG_PATH"`
